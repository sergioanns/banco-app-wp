...
function wpf_dev_process_filter( $message, $form_data, $fields, $entry_id ) {
    ...
    if ( absint( $form_data['id'] ) !== 16 ) {
        return $message;
    } 
	$url = 'http://127.0.0.1:8000/api/transfer-money';
	$request = wp_remote_post(
		$url,
		array(
			'body' => array(
				'id_origin' => $fields['2']['value'], 
				'id_destination' => $fields['4']['value'], 
				'account_balance' => $fields['3']['value']
			)
		)
	);
	...
	switch ($code) {
		case 422:
			return 'La cuenta de origen no cuenta con fondos suficientes';
		break;
		case 404:
